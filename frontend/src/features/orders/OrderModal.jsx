...
import { autoFillOrder } from '@/utils/autoFill';
...
const OrderModal = ({ isOpen, onClose, onSave, customers = [], products = [] }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setError(null); setSelectedProductId(''); setItemQuantity('1');
    }
  }, [isOpen]);
  ...
  const handleAutoFill = () => {
    const data = autoFillOrder(customers, products);
    if (!data) {
      setError('Autofill needs at least one customer and one in-stock product.');
      return;
    }
  ...
  const handleAddItem = (e) => {
    ...
    if (isNaN(qty) || qty <= 0) { setError('Quantity must be at least 1.'); return; }
    ...
    if (totalQty > activeProduct.quantity) {
      setError(`Insufficient stock for ${activeProduct.name} (${activeProduct.quantity} available, requested: ${totalQty}).`);
      return;
    }
    if (existingIndex >= 0) {
      ...
      setItems([...items, { product_id: selectedProductId, name: activeProduct.name, sku: activeProduct.sku, price: activeProduct.price, quantity: qty }]);
    }
    setSelectedProductId(''); setItemQuantity('1'); setError(null);
  };

  const handleRemoveItem = (index) => { setItems(items.filter((_, idx) => idx !== index)); setError(null); };
  ...
  const footer = (
    <>
      ...
      <Button variant="primary" onClick={handleSubmit} loading={loading} disabled={items.length === 0}>Place Order</Button>
    </>
  );
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Create New Order" footer={footer} maxWidth="700px">
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div className="form-group">
          ...
          <select id="order-customer" className="form-select" value={customerId}
            onChange={(e) => setCustomerId(e.target.value)} required disabled={loading}>
        ...
        {/* Add product row */}
        <div className="glass-card" style={{ padding: '1rem', borderStyle: 'dashed', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-end', flexWrap: 'wrap' }}>
            <div className="form-group" style={{ flexGrow: 1, margin: 0, minWidth: '200px' }}>
              <label className="form-label" style={{ fontSize: '0.8rem' }}>Select Product</label>
              <select className="form-select" value={selectedProductId}
                onChange={(e) => setSelectedProductId(e.target.value)} disabled={loading}>
            ...
            <div className="form-group" style={{ width: '100px', margin: 0 }}>
              ...
              <input type="number" min="1" className="form-input" value={itemQuantity}
                onChange={(e) => setItemQuantity(e.target.value)} disabled={loading || !selectedProductId} />
            </div>
        ...
        <div>
          ...
          <div className="table-container">
            <table className="data-table">
              <thead>
                <tr><th>Product</th><th>Price</th><th>Quantity</th><th>Subtotal</th><th style={{ width: '50px' }}></th></tr>
              </thead>
              <tbody>
                {items.length === 0 ? (
                  <tr><td colSpan="5" style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '1.5rem' }}>No products added yet.</td></tr>
                ) : (
                  items.map((item, idx) => (
                    <tr key={idx}>
                      <td>
                      ...
                      <td>
                        <button type="button" className="btn-icon" onClick={() => handleRemoveItem(idx)}
                          style={{ color: 'var(--danger)' }} disabled={loading}>
                          <Trash2 size={16} />
                        </button>
                      </td>
                    </tr>
          ...
          {items.length > 0 && (
            <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '1rem', marginTop: '1rem', paddingRight: '1rem' }}>
              <span style={{ color: 'var(--text-secondary)', fontWeight: 500 }}>Total:</span>
              <span style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--primary)' }}>{formatCurrency(calculateTotal())}</span>
            </div>
          )}
        </div>
        ...
        <div className="form-group" style={{ margin: 0 }}>
          ...
          <textarea id="order-notes" name="notes" rows="2" className="form-input"
